...
labels = ['ALLABUSE', 'NONALCILL', 'ALCOHOL', 'NONMEDPHARMA', 'SUICIDE', 'SEEKING_DETOX']

def encoding(test):
    data_onehot = test[['METRO', 'AGECAT', 'SEX', 'RACE', 'PHARMA', 'CASETYPE', 'ROUTE_1', 'TOXTEST_1', 'ROUTE_2', 'TOXTEST_2', 'ROUTE_3', 'TOXTEST_3']]

    col_count = {'METRO':14, 'AGECAT':11, 'SEX':2, 'RACE':4, 'PHARMA':2, 'CASETYPE':8, 'ROUTE_1':7, 'TOXTEST_1':2, 'ROUTE_2':7, 'TOXTEST_2':2, 'ROUTE_3':7, 'TOXTEST_3':2}
    ...
    test_encoded = test[['CASEWGT']].reset_index(drop=True)
    ...
    for feature in data_onehot.columns: 
        ...
        arr[present_value] = 1
        if feature == 'METRO' or feature == 'CASETYPE' or feature == 'PHARMA':
            arr.remove(0)
    ...
    test_encoded = pd.concat([test_encoded, data_onehot_encode, drughashencode], axis = 1)
